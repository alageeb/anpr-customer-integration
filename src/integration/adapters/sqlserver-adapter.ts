...
export class SqlServerAdapter implements CustomerDatabaseAdapter {
  ...
  private async getPool(): Promise<sql.ConnectionPool> {
    ...
    this.pool = new sql.ConnectionPool({
      server: this.config.dbHost,
      port: this.config.dbPort,
      database: this.config.dbName,
      user: this.config.dbUser,
      password: this.config.dbPassword,
      options: {
        encrypt: this.config.dbEncrypt,
        trustServerCertificate: this.config.dbTrustCertificate,
      },
  ...
  async findCustomerByPlate(
    ...
    const result = await pool
      .request()
      .input('plate', sql.NVarChar, normalizedPlate)
      .query(`
        SELECT TOP 1
        ...
        WHERE v.${mapping.plateNumberColumn} = @plate
      `);
  ...
  async getCustomerVehicles(customerId: string): Promise<VehicleRecord[]> {
    ...
    const result = await pool
      ...
      .query(`
        ...
        WHERE ${mapping.vehicleCustomerIdColumn} = @customerId
      `);
  ...
  async getCustomerStatus(customerId: string): Promise<{ status: string } | null> {
    ...
    const result = await pool
      ...
      .input('customerId', sql.NVarChar, customerId)
      .query(`
        SELECT TOP 1 ${mapping.customerStatusColumn} AS status
        FROM ${mapping.customersTable}
        WHERE ${mapping.customerIdColumn} = @customerId
      `);
